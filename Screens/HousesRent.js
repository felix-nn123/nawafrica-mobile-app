...
import { getListings, deleteAListingByUpdateStatus} from '../services/listings';

import useLocation from './../hooks/useLocation';


const HousesRent = ({navigation}) => {
...
const myData=data.filter(d=>d.rent_or_sell==="Rental"&&d.listing_type!=="Land")
...
const Listingss=["Apartment", "Studio", "Single Room", "Office", "Store/Shop","Condos"];
...
data={`${totalCount===0?"No":totalCount} Available Houses In ${selectedAfricaCountry?`${selectedCity&&selectedCountryInputs(selectedCity) + ", "}${selectedCountryInputs(selectedAfricaCountry)}`:userLocation.city&&selectedAfricaCountry?`${userLocation.city}, ${userLocation.country}`:"Nawafrica Marketplace"} For Rental`}
/>}
